...
| A story adds no classes | A story composes **default** kit components — `<Eyebrow>`, `<SectionHeader>`, `<PropertyList>` — never a raw `<h4 className="…">` or a `<div>` dressed with utilities. The one exception is a story whose subject *is* `className` passthrough, and its name says so. **Match the job, not the tag**: a small muted label is `Eyebrow`, not `TypographyH4` (which is `text-xl`). A story that needs classes is a gap in the kit — build the component there. |
| Type ladder | Three rungs, and the root is the only dial: `text-base` · `text-sm` · `text-xs` = **13 / 12 / 11** at the 13px root. Default text declares **no size**; `text-sm` is deliberately subordinate, `text-xs` is genuinely small and rare. There is no `text-meta` — it was retired as a fourth name over a second size. |
...
| **A canvas is one page per feature** | Every design canvas is organised **one page per feature** — and a feature reaches Studio as a **stacked panel**, so in practice one page per panel (`Govern › Worlds`, `Govern › Guardrails`, `Agents › Agents`, `Agents › LLMs`). Never one page per batch, per layer or per screen size. The page's name is the feature and its index number: *Govern › Worlds — 14.1*. |
| **A page carries the whole feature** | Every flow and variation of that feature lives on its page, so a reviewer sees all its use cases without opening a second canvas: the primary journey · each drawer state · each drill-in · the authoring act · **every refusal, with the bound it names** · the empty, too-few, unsupported and permission-denied states. A feature drawn only on its happy path is not drawn — but a *state* is drawn **as the UI shows it** (a struck row, a dimmed act, a marked task), never as a table explaining it. The seams **table** is the feature file's, not the artboard's. |
| **An artboard draws only what the user sees** | No annotation cards, no "what this decided and why" panels, no rationale lines, no decision-id callouts. If it would not render in Studio for a real person, it does not go on the artboard. **Every note belongs in the feature file** — that is what `docs/for-developers/` is for, and a reviewer reads the two side by side. Copy a user would actually read (an empty state's instruction, a refusal's reason, a confirm card's consequences) **is** UI and stays. |
| **An artboard is named for what it draws, and the name is the map** | `module.feature.surface.variant`, lower snake, dots for depth — `library.plans.list` · `library.plans.detail.overview` · `library.plans.detail.flow_canvas` · `library.plans.detail.flow_canvas.edit` · `library.plans.detail.flow_canvas.complex_variant1`. The **file** is that name (`library.plans.detail.overview.dc.html`) and so is the **frame title**, so an artboard, a row in `the-screens.md` and a Surfaces row in the feature file are all reachable from one string. A variant is a **deeper name, never a new surface**: two readings of one screen are `x` and `x.variant`, which is what keeps the count down. If you cannot name it this way, it is not a variant — it is a screen you have not justified. **Every board also carries one line saying what it draws**, and that line is the same string in three places: the artboard's `<title>`, its frame title in `canvas.json`, and its row in `the-screens.md`. Generate all three from one place; two descriptions of one board is a drift waiting to happen. The frame strip is canvas chrome, outside the artboard, so the line there is not a note *on* the design. |
| **A row is a name prefix** | The canvas lays out **one row per prefix**, the parent leftmost and its variants to its right: `…flow_canvas` then `…flow_canvas.complex_variant1` then `…complex_variant2`. Columns are an accident of how many variants a surface has, never a layer or a batch. |
| Where a canvas is built | `.design/canvas-<name>/` — Python generators over the shared kit (`_kit · _ui · _bpanel · _dash · _flow`, and `canvas-govern/_gov.py` for the governance vocabulary: the layer strip, participant addresses, world and rule rows). **Reuse the kit; never hand-write an artboard's HTML.** `.design/` is gitignored, so the canvas is the source and the generators are the build. |
| Canvas mechanics | Artboards are 1440×900 on the shell contract (rail + 420px `leftSection` + `mainSection`), tokens only. Each `.dc.html` carries the `data-dc-script` block with `$preview: {width: 1440, height: 900}` — without it the frame is served clipped. In `project/canvas.json`, `boards` and `order` are keyed by the **bare filename** (`Main.dc.html`, not `project/Main.dc.html`), 80px between frames in a row, one `title1` note per page. |
| The canvases | `docs/for-developers/the-screens.md` § *The design canvases* lists every canvas, what it draws and whether it supersedes another. A canvas nothing cites is a decision nobody made. |
