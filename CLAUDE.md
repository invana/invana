...
| **A canvas is one page per feature** | Every design canvas is organised **one page per feature** — and a feature reaches Studio as a **stacked panel**, so in practice one page per panel (`Govern › Worlds`, `Govern › Guardrails`, `Agents › the roster`, `Agents › LLMs`). Never one page per batch, per layer or per screen size. The page's name is the feature and its index number: *Govern › Worlds — 14.1*. |
| **A page carries the whole feature** | Every flow and variation of that feature lives on its page, so a reviewer sees all its use cases without opening a second canvas: the primary journey · each drawer state · each drill-in · the authoring act · **every refusal, with the bound it names** · the empty, too-few, unsupported and permission-denied states · and the seams table. A feature drawn only on its happy path is not drawn. |
| **One artboard, one claim** | An artboard makes a single statement and its `mainSection` argues it — the panel shows the rows, the annotation card says what the design decided and why. Title it `<page letter><n> · <the claim>`, e.g. `W3 · Authoring — picking, and two refusals that name their bound`. A title that only names a screen is a missing argument. |
