...
class JanusGraphSchemaReader:
    ...
    async def _get_graph_schema_overview(self):
        # TODO - can add more information from the print schema data like indexes etc to current output
        responses = await self.graph.execute_query("mgmt = graph.openManagement(); mgmt.printSchema()")
        return process_graph_schema_string(responses[0])

    async def _get_vertex_property_keys(self, label):
        try:
    ...
    async def _get_edge_property_keys(self, label):
        try:
            return await self.graph.execute_query(
    ...
    async def get_graph_schema(self):
        return {
            "vertices": await self.get_all_vertices_schema(),
            "edges": await self.get_all_edges_schema()
        }

    async def get_all_vertices_schema(self):
        schema_data = await self._get_graph_schema_overview()
        all_vertex_schema = {}
        for label, vertex_details in schema_data['vertex_labels'].items():
            vertex_schema = VertexSchema(**vertex_details)
            property_keys = await self._get_vertex_property_keys(label)
    ...
    async def get_all_edges_schema(self):
        ...
        for label, edge_details in schema_data['edge_labels'].items():
            ...
            property_keys = await self._get_edge_property_keys(label)
            for property_key in property_keys:
                property_schema_data = schema_data['property_keys'][property_key]
                property_schema = PropertySchema(**property_schema_data)
