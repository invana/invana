// Reusable left-rail list panel chrome (docs/for-developers/modules/explore/features/boards.md).
//
// The Sessions panel established the shape of every Explorer left rail: a
// `PanelContent` with a title, an icon-only header (refresh · search ·
...
//
// **`PanelContent`, not `TabbedPanel`.** A rail is one panel, and it was drawing
// itself as a tab strip holding a single permanent tab — a tab you cannot leave
// is not a tab. The switch pays twice over: `PanelContent` is a real flex column,
// so `footerContent` holds its height instead of overflowing the body (which is
// why the composer used to be smuggled *inside* the tab), and its title is a
// `<span>` rather than a button, so a breadcrumb in it may carry a link without
...
export interface ListPanelChromeProps {
	/** What this panel is — a word, or a trail when something is open in it. */
	title: ReactNode;
	/** A 14px lucide glyph before the title. */
	icon?: ElementType;
	...
	onClose?: () => void;
	closeLabel?: string;
	/** The close control's icon. Defaults to the left-collapse chevron, which is
	 *  right for a panel in the left rail. A panel that lives on the RIGHT — the
	 *  assistant — passes its own, because a left-pointing collapse there
	 *  says it will fold the left column, and it will not. */
...
/**
 * The shared left-rail frame. Renders a {@link PanelContent} with a title, an
...
export function ListPanelChrome({
	...
	return (
		<PanelContent
			...
			// A real footer now: `PanelContent` is a flex column, so this holds its
			// height instead of being pushed past the bottom edge.
			footerContent={footer}
			className="rounded-none border"
			headerClassName="h-[30px] pl-2"
