...
interface Props {
	...
	open: boolean;
	/** The live canvas engine (null until `<Board>` publishes it). */
...
/**
 * The Layers card. Mounted only while open — the tree subscribes to the live
 * store, so a closed card should not be listening (hence the wrapper below
 * rather than an early return inside the body).
 */
export function LayersPanel({ open, canvas, onClose }: Props) {
	if (!open) return null;
	return <LayersPanelBody canvas={canvas} onClose={onClose} />;
}

function LayersPanelBody({ canvas, onClose }: Omit<Props, "open">) {
