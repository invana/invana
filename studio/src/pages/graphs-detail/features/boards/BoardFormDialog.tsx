...
interface Props {
	 ...
	 * 1:1 canvas (docs/for-developers/modules/explore/features/graph-canvas.md), so it's owned by the session, not the canvas.
	 */
	sessionTitle: string;
...
/**
 * Edit a session's canvas: rename the session (the shared title, docs/for-developers/modules/explore/features/graph-canvas.md) and
...
export function BoardFormDialog({
	...
	useEffect(() => {
		...
		setError(null);
		if (!boardId) return;
		// Fetch the full canvas for its purpose + banner preview (docs/for-developers/modules/explore/features/graph-canvas.md); the list
		// summary is lossy/lagging, so read the source of truth directly.
		let cancelled = false;
		boardsApi
			.get(username, graphSlug, boardId)
			.then((c) => {
				if (cancelled) return;
	...
	}, [open, boardId, sessionTitle, username, graphSlug]);
	...
	async function onSubmit(e: React.FormEvent) {
		...
		try {
			...
			await update.mutateAsync({ id: boardId, data: { instructions } });
			onClose();
		} catch (err) {
			const message =
