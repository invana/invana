...
export interface GraphFormValues {
	...
	password: string;
	read_only: boolean;
	/** Optional manually-declared DB version (docs/for-developers/modules/graph-connectors/features/capabilities.md) — fallback when undetected. */
...
interface GraphFormProps {
	 ...
	 *  Required to enable Save. The version is detected from the DB (docs/for-developers/modules/graph-connectors/features/capabilities.md). */
...
export function GraphForm({
	...
	const fields: FieldConfig[] = [
		...
		},
		{
			...
			placeholder: "neo4j",
			description:
				"Which database on this server to read. Leave blank to use the connector's default; Gremlin backends address one graph per endpoint and ignore it.",
		},
	...
	const handleTest = async () => {
		...
		try {
			...
			if (result.ok) {
				...
				// see/keep it; if detection failed they type it in themselves (docs/for-developers/modules/graph-connectors/features/capabilities.md).
				if (
