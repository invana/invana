// ─────────────────────────────────────────────────────────────────────────────
// Boards API client (docs/for-developers/modules/explore/features/boards.md).
//
// The engine speaks snake_case DTOs; the Studio UI consumes the camelCase
// `Board` / `BoardSummary` shapes (with `Date`s). Mapping happens here so the
...
function toSummary(c: ApiSummary): BoardSummary {
	return {
		id: c.id,
...
export const boardsApi = {
	...
	get: async (
		...
		toBoard(await request<ApiDetail>(`${base(username, graphSlug)}/${id}`)),
