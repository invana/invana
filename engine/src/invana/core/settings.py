...
class Settings(BaseSettings):
    ...
    # How many live personal access tokens one person may hold (PT: C9). The
    # ceiling is named in the refusal, never enforced silently.
    auth_max_personal_access_tokens: int = 20
    # How often a token's `last_used_at` is written while it is in use (PT8).
    auth_token_last_used_throttle_seconds: int = 60
    # The expiries a person may pick when minting a token (PT7). Studio renders
    # the picker from this list, and creation refuses anything outside it — so a
    # deployment changes the offer in one place. "Never" is always on offer.
    auth_token_expiry_day_choices: list[int] = [30, 90, 365]
    # Username change cooldown — docs/for-developers/modules/identity-and-access/spec.md. PATCH /auth/me with a new
    # username inside the
    ...
    # Boards — version history retention (docs/for-developers/building-engine/boards-migration.md).
    # Each board keeps at most this many versions (newest wins); older ones are
    # pruned when a new one is written. 0 disables pruning (keep all).
    board_history_limit: int = 30  # INVANA_BOARD_HISTORY_LIMIT

    # TaskRuns — each graph keeps its newest N runs (steps + stream), pruned
    # when a new one opens (docs/for-developers/modules/ask/spec.md). 0 keeps all.
    run_history_limit: int = 500  # INVANA_RUN_HISTORY_LIMIT
