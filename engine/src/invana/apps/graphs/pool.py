"""ConnectionPool — the live connector pool for persisted Graph records.
...
if TYPE_CHECKING:
    ...
    from invana.apps.graphs.models import GraphConnection
...
class ConnectionPool:
    ...
    # -----------------------------------------------------------------------
    # Version compatibility (docs/for-developers/modules/graph-connectors/features/capabilities.md)
    # -----------------------------------------------------------------------
    ...
    async def _connect_graph(self, graph: GraphConnection) -> None:
        ...
        try:
            ...
            async with self._session_factory() as session:
                await GraphConnectionQuerySet().set_status(session, graph.id, "ACTIVE", latency_ms=latency_ms)
                await emit_event(
                    session,
            ...
            async with self._session_factory() as session:
                await GraphConnectionQuerySet().set_status(session, graph.id, "ERROR")
                await emit_event(
                    session,
                    action=event_actions.SYSTEM_CONNECTION_RECONNECT,
                    target_kind=event_actions.TARGET_CONNECTION,
    ...
    async def introspect(self, connection_id: str, connector: BaseConnector) -> None:
        ...
        """
        async with self._session_factory() as session:
            connection = await GraphConnectionQuerySet().get(session, connection_id)
            if connection is None:
    ...
    async def _auto_introspect(self, session: AsyncSession, graph: GraphConnection, connector: BaseConnector) -> None:
        ...
        The global model is the graph's system-managed model
        (docs/for-developers/modules/connect-and-model/features/domain-models.md): named
        ...
        try:
            ...
            await GraphConnectionQuerySet().set_schema(session, graph.id, schema.id)
            await emit_event(
                session,
...
def build_connector(graph: GraphConnection, encryption_key: str) -> BaseConnector:
    """Instantiate the vendor connector class from the persisted Graph record.

    The one place a stored connection becomes a live connector — the manager,
    the importer and `invana loader --graph` all come through here, so a Graph
    that is reachable in Studio is reachable from the CLI with no flags repeated.
    """
    ...
    # The database is only forwarded when one is named (connect-a-database.md CD8);
    # blank means the connector's own default, and Gremlin connectors address one
    # graph per endpoint and take no database at all.
    kwargs = dict(auth)
    if (graph.database or "").strip():
        kwargs["database"] = graph.database.strip()
    return ConnectorClass(uri=graph.uri, **kwargs)


# The pre-rename name. `manager` now means business logic everywhere else
# (migration-plan §7), and this is a pool: it owns live connections, a
# health-check loop and reconnect backoff, not a rule.
GraphConnectionManager = ConnectionPool
