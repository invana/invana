...
from invana.apps.graphs.compatibility import compatibility_status_for
from invana.apps.graphs.encryption import encrypt_credentials
...
class GraphConnectionQuerySet:
    ...
    async def create(
        ...
        *,
        ...
        # A manually-declared version (docs/for-developers/modules/graph-connectors/features/capabilities.md) is stored
        # as a fallback; a successful
        ...
        connection = GraphConnection(
            ...
            database=(data.database or "").strip() or None,
    ...
    async def get_or_404(self, session: AsyncSession, connection_id: str) -> GraphConnection:
        """The row, or ``NotFoundError``.

        A queryset answers *which row*, and "there is no such row" is still an
        answer — but it says so with a domain error, never an HTTP one
        (migration-plan §4.1).
        """
    ...
    async def for_graph(self, session: AsyncSession, graph_id: str) -> GraphConnection | None:
        """A Graph has at most one connection."""
        stmt = select(GraphConnection).where(GraphConnection.graph_id == graph_id)
        return (await session.execute(stmt)).scalar_one_or_none()

    async def count_for_graph(self, session: AsyncSession, graph_id: str) -> int:
        stmt = select(func.count()).where(GraphConnection.graph_id == graph_id)
    ...
    async def set_version(
        ...
        *,
        ...
        """Persist the detected/declared server version + compatibility
...
# Back-compat alias — older call sites still say GraphModelStore.
GraphModelQuerySet = GraphConnectionQuerySet
GraphModelStore = GraphConnectionQuerySet
