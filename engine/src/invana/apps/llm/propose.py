"""Natural-language → proposed graph model (docs/for-developers/modules/ask/spec.md).
...
# (docs/for-developers/modules/graph-connectors/features/capabilities.md ``PropertyType`` universal tier +
# always-available temporal). Keeping
...
def validate_proposal(proposal: ModelProposal, *, existing_node_type_names: set[str]) -> None:
    """Referential-integrity pre-check (docs/for-developers/modules/ask/spec.md) — raises before any mutation.
