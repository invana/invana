...
class TaskPlanManager:
    ...
    async def ensure_seeded(
        ...
        """
        existing = await self.querysets.key_versions(session, graph_id=graph_id)
        ...
        empty = await self.querysets.plans_without_nodes(session, graph_id=graph_id)
        for template in TEMPLATES.values():
            if (template.key, template.version) in existing:
                if (plan_id := empty.get((template.key, template.version))) is not None:
                    await self.querysets.add_tasks(session, explode(plan_id, template.steps, requires=requires))
                continue
            plan = await self.querysets.add(
                session,
                TaskPlan(
                    graph_id=graph_id,
                    key=template.key,
            ...
            await self.querysets.add_tasks(session, explode(plan.id, template.steps, requires=requires))
    ...
    async def used_by(self, session: AsyncSession, plan: TaskPlan) -> list[Agent]:
        ...
        """
        agents = await self.agents.list_active_for_graph(session, graph_id=plan.graph_id)
        return [
            agent
            for agent in agents
