"""Fixtures for boards tests — isolated Postgres schema, real DB (no mocks).

Board persistence is app-DB only; these tests never touch a graph database.
"""
...
async def db_engine():
    schema = f"test_boards_{uuid.uuid4().hex[:8]}"
