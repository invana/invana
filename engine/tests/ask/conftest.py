"""Fixtures for Ask tests — an isolated Postgres schema, real rows, no mocks.

These never touch a graph database: what is under test is the emission a step
produced and the template that chose its rendering, both of which live in the
app store.
"""
...
async def db_engine():
    ...
    schema = f"test_ask_{uuid.uuid4().hex[:8]}"
