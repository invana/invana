"""Integration tests for OpenCypher bulk queryset against a real Neo4j instance."""
...
class TestBulkDeleteVertices:
    ...
    async def test_bulk_delete_empty(self, connector):
        deleted = await connector.bulk.bulk_delete_vertices([])
        assert deleted == 0

    async def test_bulk_delete_detaches_edges(self, connector):
        v1 = await connector.data_writer.create_vertex("Detach", {"name": "X"})
        v2 = await connector.data_writer.create_vertex("Detach", {"name": "Y"})
        await connector.data_writer.create_edge("DETACH_LINK", v1.id, v2.id, {})

        count_edges_before = await connector.data_reader.count_edges("DETACH_LINK")
        assert count_edges_before == 1

        await connector.bulk.bulk_delete_vertices([v1.id])

        count_edges_after = await connector.data_reader.count_edges("DETACH_LINK")
        assert count_edges_after == 0


class TestBulkDeleteEdges:
    async def test_bulk_delete(self, connector):
        v1 = await connector.data_writer.create_vertex("Node", {"name": "E1"})
        v2 = await connector.data_writer.create_vertex("Node", {"name": "E2"})
        e1 = await connector.data_writer.create_edge("DEL_EDGE", v1.id, v2.id, {"x": 1})
        await connector.data_writer.create_edge("DEL_EDGE", v1.id, v2.id, {"x": 2})

        count_before = await connector.data_reader.count_edges("DEL_EDGE")
        assert count_before == 2

        deleted = await connector.bulk.bulk_delete_edges([e1.id])
        assert deleted == 1

        count_after = await connector.data_reader.count_edges("DEL_EDGE")
        assert count_after == 1
    ...
    async def test_bulk_delete_preserves_vertices(self, connector):
        v1 = await connector.data_writer.create_vertex("Keep", {"name": "K1"})
        v2 = await connector.data_writer.create_vertex("Keep", {"name": "K2"})
        e = await connector.data_writer.create_edge("DISCARD", v1.id, v2.id, {})

        await connector.bulk.bulk_delete_edges([e.id])

        count_vertices = await connector.data_reader.count_vertices("Keep")
        assert count_vertices == 2
