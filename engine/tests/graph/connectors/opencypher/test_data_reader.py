"""Integration tests for OpenCypher data reader queryset against a real Neo4j instance."""
...
class TestReadVertices:
    ...
    async def test_read_with_offset_and_limit(self, connector, seeded_graph):
        await connector.data_reader.read_vertices("Person")
    ...
    async def test_read_with_nested_filter(self, connector, seeded_graph):
        """(age > 28 AND city = NYC) OR name = Bob"""
        filters = FilterGroup(
            operator=LogicalOp.OR,
            conditions=[
                FilterGroup(
...
class TestShortestPath:
    ...
    async def test_shortest_path_indirect(self, connector, seeded_graph):
        bob = seeded_graph["bob"]
        charlie = seeded_graph["charlie"]
        # Bob -> Alice -> Charlie (2 hops)
        path = await connector.data_reader.shortest_path(bob.id, charlie.id)
        assert path is not None
...
class TestCountVertices:
    ...
    async def test_count_empty_label(self, connector, seeded_graph):
        count = await connector.data_reader.count_vertices("NonExistent")
        assert count == 0
...
class TestCountEdges:
    ...
    async def test_count_empty_label(self, connector, seeded_graph):
        count = await connector.data_reader.count_edges("NONEXISTENT")
        assert count == 0
