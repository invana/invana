"""Fixtures for the Skills suite — isolated Postgres schema, real DB (no mocks)."""
...
async def db_engine():
    schema = f"test_skills_{uuid.uuid4().hex[:8]}"
...
@pytest_asyncio.fixture
async def other_graph(session: AsyncSession, user: User) -> Graph:
    row = Graph(slug="other", name="Other", created_by_id=user.id, setup_state={})
    session.add(row)
    await session.flush()
    return row
