...
export const pubmedExampleDataset1: ICanvasData = {
  nodes: exampleDataset1Raw.nodes.map(node => {
    // Destructure the known fields that become top-level properties.
    // Remove 'id', 'ox', 'oy' (and optionally 'group') from the rest.
    const { id, ox, oy, group, grabbable, removed, selectable, selected, locked, classes, ...otherProps } = node;
    return {
      id,
      type: group,  // fixed type for canvas nodes
      // Optionally include 'group' here or leave it in otherProps.
      properties: { group, ...otherProps }
    };
  }),
  edges: exampleDataset1Raw.edges.map(edge => {
    // For edges, we extract id, source, target.
    // We also take the original "type" from the input (the relationship type)
    // and move it into properties under a new key (for example "relationType")
    // so that our top-level "type" can be used to flag canvas elements as "edge".
    const { id, source, target, group, grabbable, removed, selectable, selected, locked, classes,
      type: relationType, ...otherProps } = edge;
    return {
      id,
      type: relationType,  // fixed type for canvas edges
      source,
      target,
      properties: { group, relationType, ...otherProps }
    };
  })
};
