...
| G13 | `bottomSection` is the console, with `bottomSpan: 'main'` — it spans the canvas only, so `leftSection` and `rightSection` stay full height |
| G16 | **One param per region, and its value is the occupant**: `?panel=` is the left column's open section, `?page=` is what fills `mainSection`, `?right=` is who holds `rightSection` — `assistant` · `inspector`, absent means closed. The breadcrumb reads the URL back literally — `owner › graph › panel › object`, the panel crumb being the `?panel` value verbatim. `?settings=` was this param's first name and is still **read**, never written; so are `?ai=` and `?inspector=open` |
...
| G29 | **LLM providers are a Settings tab, not a rail icon.** A provider is the Graph's own configuration in exactly the way the connection is — one is the database it reads, the other the model it thinks with — and `leftNav` is for things a person *goes to* (G2). It cost an icon to say what a tab says. Inside the tab the panel keeps its list → detail shape: the breadcrumb and **Add** move from a panel header into the tab body, because one panel has one tab strip and never a second header row under it. Search goes with the chrome — a Graph has two or three providers, and the status bar already counts them. `?panel=llms` still resolves, now onto `settings` with the `llms` tab open, so every bookmark and every setup step lands where it always did |
...
| `explorer/components/{ExplorerCanvas,ExplorerTypesPanel,InspectorPanel,LayersPanel,ExpandFineTunePanel,StylingPanel,RendererCapabilityBanner}` · `hooks/useExpandNode` · `lib/{canvasTheme,typeColor,visibility}` | `explorer/` — what is drawn, and what is selected on it |
