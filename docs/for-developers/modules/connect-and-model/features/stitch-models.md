...
| ST24 | **Models, with nothing selected, *is* the landscape.** The main area belongs to the open panel, so the Models panel's two views each own a canvas: the **list** view's is *All models*, the **detail** view's is that model. Switching the `leftNav` to Models therefore draws every model at once — the whole landscape, before anyone has chosen where to look — and coming back to the list from a model returns to it. Picking a model replaces it; closing the page with its X leaves it closed, and the Models drawer header's icon is how you get it back. There is no "open all models on one canvas" row: a canvas you have to ask for is a canvas most people never see. |
...
| ST37 | **Counts live on the panel, never on the canvas.** `5 models`, `6 stitches`, `2 staged` read on the Models panel's status bar, and each drawer's own count rides its header so it is legible while the drawer is closed (ME13). A count in two places is a count that can disagree with itself, and the panel is where a person is already reading the list those counts are about. The canvas keeps only what is about the *drawing*: the legend, the altitude, and the banner for a stitch bound to a version it is not drawing. |
...
| ST53 | **A stitch touches the model version on each side of it.** Solving writes edges between two models, so the step that solves (an import's `stitch` — ST47 — or `commit_stitches` — ST44) engages both sides' published versions, `graph_data/model/<Name>@<version>`, and records one touch per model with the edges written into it ([GV35](../../govern/spec.md#4-cross-feature-decisions)). An import that loads Tweets and solves `tweet_article@about` touches `Tweets@1.0.0` and `Articles@1.0.0`, and the dashboard's *What each step engaged* shows both on the `stitch` row. A model the step checked but wrote nothing into gets no touch: a load's `stitch` checks every active stitch, and one scoped to records this load did not write merges nothing. A side a guardrail refuses skips that stitch — struck in place, the others still solved — because the records the load already wrote are not wrong for it. |
| ST54 | **Committing is a run, whoever commits.** The Stitches drawer and `invana stitches commit` / `apply --commit` open `stitch-commit@1` — one step, `commit_stitches` — under the Graph's guardrails (no agent, no world, as a load), and the drawer awaits it. Nothing else calls `stitching.commit_stitches`, and its `admit` is required, so no path writes a stitch's edges without asking the lens. Nothing staged, no connection and a read-only connection refuse before a run exists. `commit_stitches` declares no `requires`: what it commits is the staged set, and staging is usually a person's earlier act rather than a step in this plan. |
| ST55 | **Removing an active stitch is a run, and a refusal keeps the rule.** Its edges are withdrawn by `stitch-withdraw@1` — one step, `withdraw_stitch` — under the Graph's guardrails, and the rule row goes only once they are gone. A guardrail denying either model refuses the whole removal, naming the rule: withdrawing the rule while keeping its edges would leave edges that name a stitch nobody can see, which is the one outcome ST48 exists to prevent. A connection that is missing, read-only or unreachable refuses or fails the same way — the rule stays until its edges can go. A **staged** stitch never wrote an edge, so removing it deletes the row and opens no run. |
| ST56 | **Previewing a rule is a run, because counting reads the graph.** The declare card's count and `invana stitches resolve` open `stitch-preview@1` under the Graph's guardrails; `preview_stitches` (`graph_read`) sends the preview and written-edge counts through the run's lens like any other read. A rule naming a type or key the guardrails deny comes back **refused, with the reason** — `409 preview_refused` in the drawer, `refused — …` in the terminal — never counted around. |
