...
| ME15 | **Node types and Edge types open; the rest arrive closed.** They are the two lists the panel exists for and the two the hi-fi draws. Stitches and Staged are a run of closed headers beneath them, one click from open. Four open sections in a 300px rail give each one a couple of visible rows, which is the fold problem again in a new shape. |
...
| ME17 | **The model list and a model's detail are two views, and the crumb is the seam.** The panel shows the list until one is picked, then that model's drawers — one thing on screen at a time, each getting the whole column. `Models › Observations` is the panel's title, and **`Models` is a link**: the breadcrumb is the way back, which is what a breadcrumb is for. |
| ME18 | **The crumb lives in the panel header.** `Models › Observations` is the panel's title, and `Models` is a link out of it. That is legal because the rail is a `PanelContent`, whose title is a `<span>`; while it was a one-tab `TabbedPanel` the label sat inside a button and a link there would have nested one interactive element in another. Beneath it the detail opens straight into `ModelMetaLine` (ME21) — no second header row. |
...
| ME21 | **A model's own metadata reads on one line, with `More` for the rest.** The panel's column belongs to the type lists (ME13/ME15), so the model's description, validation mode, origin, status, version and dates cannot each take a row. The description — the one field a person scans — reads truncated to a single line above the stack; `More` expands the rest as a `PropertyList` and `Less` puts it back. The version chips (`VersionBar`) are the `Version` row's value rather than a row of their own above the stack — two chips that change once per publish do not earn the full width of the rail. Collapsed height is constant across models, so switching models never moves the drawers. There is no author row: `GraphModel` records no creator, and a row that always said `—` is worse than no row. |
