...
**Studio**
- Each settings section is a top-level icon in the page's `leftNav` rail: Info · Intent · LLMs · Skills · Instructions · Datasets · Members. Defined once in `useGraphLeftNav(username, graphSlug, activeTab)` and consumed by Overview, Explorer, and Modeller — so every graph page exposes the same chrome.
