...
- **Model** = **`GraphModel`** — a persona's model of the data: node/edge types, properties, constraints, indexes, with version history. This is the entity this RFC multiplies. "The architecture model," "the test model" are each one `GraphModel` row.
