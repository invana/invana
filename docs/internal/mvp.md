...
- **Deferred:** [-] constraints/indexes authoring UI · YAML round-trip of authored models. (Modeller-canvas layout positions stay ephemeral here; **Explorer** canvas persistence — snapshot + positions + view — is designed separately in § 5.8 / RFC-043.)
...
- **Backend:** [x] `sessions.surface` (`explorer`|`modeller`, default explorer) + `sessions.model_id` (nullable FK, SET NULL) · surface-branch in `send_message` (modeller → ensure model+draft → `propose_model` forced tool → referential-integrity validate → reconcile into the draft via `ModelStore`) · `model.generate` event · messages stay metadata-only (summary + "Added N/M/K" composed into `content`) · commit reuses `POST /models/{id}/versions/{vid}/activate` (no new route) · `surface` filter on the sessions list.
- **Frontend:** [x] Modeller Sessions panel (reuse `SessionsPanel`/`SessionComposer`, `surface=modeller`, NL only) via a new single-open left-rail `messages` native key · assistant change-summary in the reply `content` · "Commit" → existing activate mutation (publish-confirm) · session's bound `model_id` syncs the canvas · invalidate the `["models", u, g]` subtree after generation so tree+canvas refresh.
...
### 5.5a Explorer node expand / progressive disclosure — RFC-035
Right-click a node to pull its neighbours from the bound DB — all neighbours, by neighbour **node type**, or by **edge type** + direction (incoming/outgoing/both). Fine-tunable so 1000+ neighbour nodes load only the meaningful slice: pagination (page size + offset), sorting (by property asc/desc), property filters. Both Cypher + Gremlin.
- **Backend:** [ ] `SortSpec` type · `ORDER BY` / `.order().by()` + `neighbor_label`/`filters`/`offset`/counts on the connector `match_neighbors` builders · focused `data_reader` methods (`read_neighbors`, `read_neighbors_by_edge_type`, `read_neighbors_by_node_type` + `count_*`) · new `invana/explorer/` module: three POST routes `/explorer/expand/{neighbors,by-edge-type,by-node-type}` over the connector `data_reader` (read-only, reuse `query_service._resolve_connector`), returning `{data, total, offset, limit, returned, has_more}` · `graph.expand` audit event.
- **Frontend:** [ ] schema-driven `Expand ▶` group in the Explorer node context menu (by node type / incoming / outgoing) · `ExpandFineTunePanel` (page size · sort · filters · "Load next page" · "Showing X of N") · results **merge** into the canvas (dedupe by id) · per-node pagination state.
- **Integrations:** existing graph DB connectors · `@invana/canvas-react` context menus · `@invana/design-kit` panel · `useActiveVersionQuery` schema. Detail: [`mvp/rfc-035-explorer-node-expand.md`](mvp/rfc-035-explorer-node-expand.md).

### 5.5b Explorer model browser (read-only)
- **Frontend:** [x] Read-only, file-manager-style schema browser docked in the Explorer left rail under a new `?settings=model` native key (`ListTree` rail icon, Explorer-only): node/edge types → Properties / Indexes / Constraints → data types + cardinality. Reads `useActiveVersionQuery`; all authoring stays in the Modeller. `GraphDetail` generalized so a view can own multiple native left-rail panels (Explorer: `sessions` + `model`).
...
### 5.8 Explorer canvases — saved views + tabs (RFC-043)
Persist a **canvas** — the painted graph snapshot, viewport, filters, node positions, settings — plus a title and written *purpose (instructions)*. Canvases are **shared graph-wide** and each is backed **1:1 by a Session** (`canvas.session_id` unique/NOT NULL/CASCADE); the canvas is self-contained (own snapshot + copied `source_query`) so members render it without reading the private backing thread. Surfaced as an Explorer **left-rail panel** (paginated list · edit / delete / archive / open) **and** a **main-section tab bar** (canvas toolbar stays in the app header above) — each open canvas is a tab; "+" starts a blank canvas (new session + canvas) and selecting a tab switches the active query session (queries belong to the active canvas). Detail: [`mvp/rfc-043-explorer-canvases.md`](mvp/rfc-043-explorer-canvases.md).
- **Backend:** [x] `invana.canvases` module (model/store/services/schemas/routes) mirroring `sessions/` · `Canvas` (1:1 `session_id`, denormalized `graph_id`, `snapshot`/`source_query`/`view_state`/`filters`/`positions`/`settings` JSON, `pinned`/`archived`) · paginated CRUD under `/api/v1/u/{username}/{graphSlug}/canvases` (shared — `require_graph_member`, not creator-filtered; `?include_archived`) · Alembic revision · `CanvasView` admin · `canvas.*` events + `canvas` target kind.
- **Frontend:** [x] `"canvases"` native left-rail section (`SettingsSection` + `NATIVE_SECTIONS.explorer` + rail icon) · `CanvasesPanel` (paginated list · edit / delete / archive / open) · `CanvasTabsBar` (main-section tabs + "+" new-canvas + toolbar) · `CanvasFormDialog` (title + instructions) · `services/api/canvases.ts` + `hooks/queries/useCanvases.ts` · `ExplorerPage` hydrate/autosave + tab↔session unification.
- **Integrations:** none new — `@invana/ui` (`Dialog`/`DropdownMenu`/list) · `@tanstack/react-query` · `@invana/canvas-react` store APIs.
- **Version history + live banner (RFC-047):** [ ] Live banner — 10s throttled autosave refreshes the sessions-list preview while a canvas is open (not only on blur). [ ] `canvas_states` append-only table (per-turn `canvas.exportState()` snapshot + `exportDataURL` thumbnail + `kind`/`label`/counts + nullable `message_id`); keep newest N (default 30, `INVANA_CANVAS_HISTORY_LIMIT`); `POST`/`GET` list+detail (restore is client-side via `importState`, no fork endpoint); `CanvasStateView` admin; Alembic 026. [ ] Studio: capture a version at each canvas-mutating turn (query/expand/load) + manual "Save current state", header **History** timeline with lazy thumbnails, "Open as new canvas" restore via `importState`. Uses `@invana/canvas` ≥ 0.0.11 export/import. Detail: [`mvp/rfc-047-canvas-version-history.md`](mvp/rfc-047-canvas-version-history.md).
