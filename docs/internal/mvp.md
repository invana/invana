...
- **Version history + live banner (RFC-047):** [ ] Live banner — 10s throttled autosave refreshes the sessions-list preview while a canvas is open (not only on blur). [ ] `canvas_states` append-only table (per-turn snapshot + positions + banner thumbnail + `kind`/`label`/counts + nullable `message_id`); keep-all; `POST`/`GET` list+detail + `POST …/states/{sid}/fork` (restore = open as a new session+canvas); `CanvasStateView` admin; Alembic 026. [ ] Studio: capture a version at each canvas-mutating turn (query/expand/load), header **History** timeline with lazy banner thumbnails, "Open as new canvas" fork. Detail: [`mvp/rfc-047-canvas-version-history.md`](mvp/rfc-047-canvas-version-history.md).
