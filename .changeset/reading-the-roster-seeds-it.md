...
roster had no test of its own, which is how a one-character slip shipped; `tests/agents` now
has a suite that seeds a graph by listing it and proves a second list does not duplicate the
roster.
