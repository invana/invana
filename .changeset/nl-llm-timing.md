...
- The LLM translation step is traced (`llm.translate` / `llm.generate` spans with model id and token counts), so it shows up in the same FE→BE trace as `graph.query.db_execute` (docs/for-developers/modules/platform/features/telemetry.md).
