...
When a natural-language ask is genuinely ambiguous — a likely typo of a label/property/value, a reference it can't resolve, or something the schema can't express — translation now returns a short clarifying question instead of guessing a query (docs/for-developers/modules/ask/features/clarifying-questions.md). The question is persisted as a normal assistant reply; the user answers in the composer and the clarification exchange is replayed as conversation context (docs/for-developers/modules/ask/spec.md), so the model remembers what it asked.
